...
namespace MadsKristensen.AddAnyFile
{
    public static class ProjectHelpers
    {
        ...
        public static ProjectItem AddFileToProject(this Project project, FileInfo file, string itemType = null)
        {
            ...
            // Appears to be a bug in project system or VS, so need to make sure the project is
            // aware of the folder structure first, then add the time to that folderStructure
            // (create and/or find) if adding to the root ProjectItem then just do that.
        ...
        public static ProjectItem AddFolders(Project project, string targetFolder)
        {
            ...
            ProjectItem existing = project.ProjectItems.Cast<ProjectItem>().FirstOrDefault(i =>
            {
                Microsoft.VisualStudio.Shell.ThreadHelper.ThrowIfNotOnUIThread();
                return i.Name.Equals(list.First(), StringComparison.OrdinalIgnoreCase);
            });
            ProjectItem item = existing ?? project.ProjectItems.AddFolder(list[0]);

            foreach (string folder in list.Skip(1))
            {
                existing = item.ProjectItems.Cast<ProjectItem>().FirstOrDefault(i =>
                {
                    Microsoft.VisualStudio.Shell.ThreadHelper.ThrowIfNotOnUIThread();
                    return i.Name.Equals(folder, StringComparison.OrdinalIgnoreCase);
                });
